...
module.exports = function(args) {
	...
	var input = {
		title: args._ || '',
		to   : args.t || args.to   || null,
		all  : args.a || args.all  || null, // true or false
	};

	this.config.render_drafts = true;

	var source_dir = this.source_dir;

	// conner case
	if (typeof(input.to) === 'boolean') {
		console.log(chalk.red('Error: '), 'wrong usage of "-t|--to" flag. go to "hexo help" for usage.');
		return;
	}

	var destDir = input.to===null? path.join(source_dir, '_posts'): path.join(source_dir, input.to);
			...
			if (input.title.length !== 0) {
				...
				return allPosts.filter(function(post) {
					return regexTitle.every(function(regex) {
						return (regex.test(post.title)||regex.test(post.slug))&&!post.published;
					});
				});
			} else { // all flag is up
						...
						inquirer.prompt([
							{
								type: 'list',
								name: 'post',
								message: 'Select the post you wish to publish/undraft.',
								choices: choices,
							},
		...
		.then(function checkFolderOrCreate(passOn) {
			passOn.map(function(post) {
				...
				try {
					...
					console.log(chalk.red('Error: '), 'cannot mkdir of given path');
				}
			});
			return passOn;
		})
				...
				inquirer.prompt([
					{
						type: 'confirm',
						name: 'confirm',
						message: 'Are you sure you want to publish/undraft all posts in _drafts?\n  Make sure you have checked each individual post having "dest" parameter',
					},
				], function getAnswer(answer) {
					if (!answer.confirm) {
